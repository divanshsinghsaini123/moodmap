...
/** shape of properties in countries geojson used */
type CountryProps = {
  name?: string;
    ...
    <div className="relative w-full max-w-6xl mx-auto rounded-2xl border border-slate-600 bg-slate-800 p-6">
      ...
      >
        <Geographies geography={GEO_URL}>
          {({ geographies }) =>
            geographies.map((g) => {
              // g is of unknown library type — cast safely to our typed shape
              ...
              const stat = code ? stats[code] : undefined;
              const name = props.name ?? code ?? "Unknown";

              // compute ratio and fill
              let fill = "#0f172a";
              if (stat) {
                const good = stat.good || 0;
                const bad = stat.bad || 0;
                const ratio = good + bad === 0 ? 0 : (good - bad) / (good + bad);
                fill = colorForRatio(ratio);
              }
